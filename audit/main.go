...
func main() {
	...
	out := os.Stdout
	fmt.Fprintln(out, "# PVE-defaults audit (issues #199 + #178)")
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "Generated by `audit/main.go`. Two rules combined:")
	fmt.Fprintln(out, "1. **Defaults rule (#199)**: Go zero value differs from the documented PVE default → send the wrong value when unset.")
	fmt.Fprintln(out, "2. **Type rule (#178)**: PVE schema declares `\"type\": \"boolean\"` but the Go field is plain `int`/`uint`/`bool` instead of `IntOrBool` → wrong type on the wire.")
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "Recommendation matrix: schema=bool + defaults match → `IntOrBool`; schema=bool + defaults differ → `*IntOrBool`; otherwise → `*T` only when defaults differ.")
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "> **Note on `FirewallNodeOption.Enable` vs `FirewallVirtualMachineOption.Enable`**: per-node firewall defaults to `1` (ships enabled), per-VM defaults to `0` (opt-in). This asymmetry is intentional in PVE's three-gate firewall design (cluster + node + VM) and is *not* a schema bug.")
	fmt.Fprintln(out, "")
	...
	if len(missingEndpoints) > 0 {
		fmt.Fprintln(out, "## ⚠ Endpoints not found in schema (audit incomplete for these)")
	...
	for _, t := range targets {
		...
		fmt.Fprintf(out, "## `%s`\n", t.StructName)
		if usedEndpoint != "" {
			fmt.Fprintf(out, "Schema source: `%s`\n\n", usedEndpoint)
		}
		if len(flagged) == 0 {
			fmt.Fprintln(out, "_No mismatches found._")
		} else {
			fmt.Fprintln(out, "| Field | json tag | Go type | PVE type | Go zero | PVE default | Recommended fix |")
			fmt.Fprintln(out, "|-------|----------|---------|----------|---------|-------------|-----------------|")
			...
			for _, r := range flagged {
				...
				fmt.Fprintf(out, "| `%s` (L%d) | `%s` | `%s` | `%s` | `%s` | `%s` | %s |\n",
		...
		if len(untracked) > 0 {
			...
			for _, u := range untracked {
				fmt.Fprintf(out, "- `%s` (json: `%s`, type: `%s`)\n", u.Name, u.JSONTag, u.GoType)
			}
			fmt.Fprintln(out, "\n</details>")
		}
		fmt.Fprintln(out, "")
